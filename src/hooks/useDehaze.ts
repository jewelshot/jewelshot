/**
 * Custom hook for professional dehaze processing
 * Applies Dark Channel Prior algorithm for atmospheric haze removal
 */

import { useState, useEffect, useRef, useCallback } from 'react';
import { applyDehaze, dehazeValueToParams } from '@/utils/dehaze';

interface UseDehazeOptions {
  /**
...
/**
 * Hook for applying professional dehaze
 * Uses Dark Channel Prior algorithm
 *
 * @param options - Configuration options
 * @returns Processed image result
 */
export function useDehaze(options: UseDehazeOptions): UseDehazeResult {
  const { originalSrc, dehaze, enabled, highQuality = false } = options;
  ...
  const processImage = useCallback(async () => {
    ...
    try {
      ...
      // Calculate optimal dehaze parameters
      const params = dehazeValueToParams(dehaze, highQuality);

      // Apply dehaze
      const processed = applyDehaze(imageData, params);
      ...
      console.error('Dehaze error:', err);
  ...
  }, [originalSrc, dehaze, enabled, highQuality, processedSrc]);
  ...
  useEffect(() => {
    ...
    // Debounce processing (1000ms for dehaze - ultra smooth slider)
